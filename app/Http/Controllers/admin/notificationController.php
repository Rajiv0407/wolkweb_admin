...
use App\model\notification_fors;
use App\model\notification_types;
use App\model\announcement_lists ;
use DB ;
...
class notificationController extends Controller
{
   public function index(Request $request){
    ...
    public function notify_datatable(){
        ...
        $carQry="select al.id as id ,al.title,content , deviceType,case when al.type is null then '' else (select title from pe_announcement_master where id=al.type) end as nType , case when nFor is null then '' else (select title from pe_announcement_for where id=al.nFor) end as nFor,   case when al.status=1 then 'Active' else 'Inactive' end as status_ , al.status from pe_announcement_list as al" ;
